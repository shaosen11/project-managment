...
public class ProjectsIndexController {
    ...
    public String getProjectsIndex(@PathVariable("userId") Integer id,Model model) {
        ...
        String recommendUser = id.toString();
        ...
        if (list.size()>=0){
            ...
            model.addAttribute("projects", list1);
    ...
    //我的项目--收藏
    @GetMapping("/projects_index_refrech/{userId}/{projectId}")
    public String myProjectsRefrech(@PathVariable("userId") Integer id, @PathVariable("projectId") Integer projectId, Model model) {
        ...
        //重定向回推荐项目页面
        return "redirect:/projects_index/"+id;
    }

    //我的项目--取消收藏
    @GetMapping("/projects_index_del_refrech/{userId}/{projectId}")
    ...
    //统计点击量
    @GetMapping("/projects_index_click/{userId}/{projectId}")
    public String getProjectsClick(@PathVariable("userId") Integer id, @PathVariable("projectId") Integer projectId, Model model) {
        boolean flag = projectService.updateProjectClickNumber(projectId);
        ...
        userClick1.setUserId(id);
        userClick1.setProjectsId(projectId);
        userClick1.setClickTime(now);
        System.out.println("添加UserClick" + userClick1);
